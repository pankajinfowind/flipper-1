...
const CUSTOM_ICONS_PATHS = [
    ...
    YEGO_CLIENT_PATH + 'assets/icons/*.svg'
];
...
gulp.task('i18n-extract', function () {
    ...
    fs.writeFile(
        './../server/resources/client-translations.json',
...
gulp.task('dist', function () {
    ...
    gulp.src(['./../dist/**/*', '!./../dist/index.html', '!./../dist/stats.json']).pipe(
        gulp.dest('./../server/public/client')
    );
...
function getIconNames() {
    ...
    htmlFiles = htmlFiles.concat(getFilesInPath(YEGO_CLIENT_PATH, '.html'));

    var names = extractIconNamesFromHtmlFiles(htmlFiles);
    names = names.concat(
        extractIconNamesFromTsFiles(getFilesInPath(CLIENT_APP_PATH, '.ts'))
    );
