...
const FLIPPER_APP_PATH = './src/app/';
const FLIPPER_ICONS_PATH = './src/assets/icons/';
const YEGO_FLIPPER_PATH = './src/common/';
const NODE_MODULES_PATH = './node_modules/';
const CUSTOM_ICONS_PATHS = [
    './src/assets/icons/individual/*.svg',
    YEGO_FLIPPER_PATH + 'assets/icons/*.svg'
];


function extractTranslationsFromTsFiles() {
    var files = getFilesInPath(FLIPPER_APP_PATH, '.ts');
    ...
    files.forEach(function (path) {
        ...
        var tsFile = ts.createSourceFile(
            path,
        ...
        nodes.forEach(function (node) {
            ...
            if (
                !propAccess ||
                !propAccess.name ||
                props.indexOf(propAccess.name.text) === -1
            )
                return;
            if (!methodAccess || !methodAccess.name || methodAccess.name.text !== 'show')
                return;
            ...
            var keys = _extractTranslationsStringsFromFunctionCallExpression(
                node,
                argIndex
            );
...
gulp.task('i18n-extract', function () {
    var files = getFilesInPath(FLIPPER_APP_PATH, '.html');
    var fromHtml = {};
    ...
    files.forEach(function (path) {
        ...
        $(
            '[trans], [trans-placeholder], [trans-title], [itemsName], [tooltip], [matTooltip]'
        ).each(function (i, el) {
            var $el = $(el),
                text;
    ...
    fs.writeFile(
        '../flipper-api/resources/flipper-translations.json',
...
gulp.task('dist', function () {
    ...
    //gulp.src('./../server/public/FLIPPER', {read: false}).pipe(clean({force: true}));
    rimraf.sync('../flipper-api/public/flipper');

    //copy dist folder into laravel public folder
    gulp.src(['./dist/**/*', '!./dist/index.html', '!./dist/stats.json']).pipe(
        gulp.dest('../flipper-api/public/flipper')
    );
    ...
    var laravelViewPath = '../flipper-api/resources/views/home.blade.php';
...
gulp.task('svgstore', function () {
    ...
    //filter out svg icons that are not used in project
        ...
        .pipe(gulp.dest(FLIPPER_ICONS_PATH));
});
...
gulp.task('normalize-icon-names', function () {
    return gulp
        .src(CUSTOM_ICONS_PATHS)
        .pipe(clean())
        .pipe(
            rename(function (file) {
                if (file.basename.indexOf('-custom') === -1) {
                    file.basename =
                        file.basename.toLowerCase().replace(/[_\s]/, '-') + '-custom';
                }
            })
        )
        .pipe(gulp.dest(FLIPPER_ICONS_PATH));
...
function getIconNames() {
    var htmlFiles = getFilesInPath(FLIPPER_APP_PATH, '.html');
    htmlFiles = htmlFiles.concat(getFilesInPath(YEGO_FLIPPER_PATH, '.html'));

    var names = extractIconNamesFromHtmlFiles(htmlFiles);
    names = names.concat(
        extractIconNamesFromTsFiles(getFilesInPath(FLIPPER_APP_PATH, '.ts'))
    );
