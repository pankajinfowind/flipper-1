...
export class CurrentUser {
  ...
  constructor(private eventBus: FlipperEventBusService, private model: ModelService, private database: PouchDBService) {
  ...
  public async defaultBusiness(userId) {
    await this.database.activeBusiness(userId).then(
      (res: { docs: string | any[] }) => {
        console.log(res)
        if (res.docs && res.docs.length > 0) {
  ...
  public async defaultBranch() {
    if (this.currentBusiness) {
      await this.database.activeBranch(this.currentBusiness.id).then(
        (res: { docs: Branch[] }) => {
