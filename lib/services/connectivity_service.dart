...
import 'abstractions/stoppable_service.dart';
...
class ConnectivityService extends StoppableService {
  ...
  StreamController<ConnectivityStatus> connectionStatusController = BehaviorSubject();
  ...
  final Logger log = Logging.getLogger('Connectivity Status Service');
    ...
    return Connectivity().onConnectivityChanged.listen((ConnectivityResult result) async {
      ...
      if (Platform.isAndroid) {
        ...
        if (result != ConnectivityResult.none) {
          try {
            ...
            await http.get('https://google.com') // However 'https://jsonplaceholder.typicode.com/todos/1' seems the fastest after some empirical tests
      ...
      } else { // on iOS proceed as expected
        connectionStatusController.add(_getStatusFromResult(result));
      }
