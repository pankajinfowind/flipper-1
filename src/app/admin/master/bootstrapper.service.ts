...
export class Bootstrapper {
    ...
    protected categories(): Promise<Category[]> {
        ...
        return new Promise((resolve, reject) => {
            this.http.get(url)
                .pipe(finalize(() => this.modelMasterService.update({loading: false})))
                .subscribe(
                    res => {
                                this.modelMasterService.update({loading: false, categories: res['categories']['data']?res['categories']['data']:[]});
    ...
    protected items(): Promise<Item[]> {
      ...
      return new Promise((resolve, reject) => {
          this.http.get(url)
              ...
              .subscribe(
                  res => {
                              this.modelMasterService.update({loading: false, items: res["items"]["data"]?res["items"]["data"]:[]});
    ...
    protected insurances(): Promise<Insurance[]> {
      let url;
      if (this.settings.getBaseUrl() != "http://localhost:4200/") {
          url = AppConfig.url + "secure/" + API_ROUTES_INSURANCES.BUSINESS_INSURANCES;
      } else {
          url = this.settings.getBaseUrl() + "secure/" + API_ROUTES_INSURANCES.BUSINESS_INSURANCES;
      }
      ...
      return new Promise((resolve, reject) => {
          this.http.get(url)
              ...
              .subscribe(
                  res => {
                              this.modelMasterService.update({loading: false, insurances:  res['business_insurance']?res['business_insurance']:[]});
